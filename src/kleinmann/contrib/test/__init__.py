...
_KLEINMANN_TEST_DB = "sqlite://:memory:"
...
async def _init_db(config: dict) -> None:
    ...
    await Kleinmann.generate_schemas(safe=False)
...
def _restore_default() -> None:
    ...
    Kleinmann._init_apps(_CONFIG["apps"])
    Kleinmann._inited = True
...
def initializer(
    ...
    global _CONNECTIONS
    global _LOOP
    global _KLEINMANN_TEST_DB
    global _MODULES
    global _CONN_CONFIG
    _MODULES = modules
    if db_url is not None:  # pragma: nobranch
        _KLEINMANN_TEST_DB = db_url
...
class IsolatedTestCase(SimpleTestCase):
    ...
    If you define a ``kleinmann_test_modules`` list, it overrides the DB setup module for the tests.
    """

    kleinmann_test_modules: Iterable[Union[str, ModuleType]] = []

    async def _setUpDB(self) -> None:
        await super()._setUpDB()
        config = getDBConfig(app_label="models", modules=self.kleinmann_test_modules or _MODULES)
        await Kleinmann.init(config, _create_db=True)
        await Kleinmann.generate_schemas(safe=False)
...
class TruncationTestCase(SimpleTestCase):
    ...
    async def _tearDownDB(self) -> None:
        ...
        for app in Kleinmann.apps.values():
...
def init_memory_sqlite(
    ...
    .. code-block:: python3
        ...
        from kleinmann import fields, models, run_async
    ...
    def wrapper(func: AsyncFunc, ms: List[str]):
        ...
        async def runner(*args, **kwargs) -> T:
            await Kleinmann.init(db_url=MEMORY_SQLITE, modules={"models": ms})
