...
class RegisterKleinmann(AbstractAsyncContextManager):
    """
    Registers Kleinmann-ORM with set-up and tear-down
...
def register_kleinmann(
    ...
    """
    Registers ``startup`` and ``shutdown`` events to set-up and tear-down Kleinmann-ORM
    ...
    if isinstance(lifespan := app.router.lifespan_context, _DefaultLifespan):
        ...
        # So people can upgrade kleinmann-orm in running project without changing any code
        ...
        # If custom lifespan was passed to app, register kleinmann in it
        warnings.warn(
            "`register_kleinmann` function is deprecated, "
            "use the `RegisterKleinmann` class instead."
            "See more about it on https://kleinmann.github.io/examples/fastapi",
            DeprecationWarning,
        )
