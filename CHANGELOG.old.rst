...
Added
^^^^^
- Propagate `_create_db` parameter to RegisterKleinmann. (#1676)
...
- Add argument use_tz and timezone to RegisterKleinmann. (#1649)
- Support await `kleinmann.contrib.fastapi.RegisterKleinmann`. (#1662)
- Add `kleinmann.contrib.test.init_memory_sqlite`. (#1657)
...
- Only `Model`, `Kleinmann`, `BaseDBAsyncClient`, `__version__`, and `connections` are now exported from `kleinmann`
...
- Added `Microsoft SQL Server`/`Oracle` support, powered by `asyncodbc <https://github.com/kleinmann/asyncodbc>`_, note that which is **not fully tested**.
...
- Added a new unified and robust connection management interface to access DB connections which includes support for
  lazy connection creation and much more. For more details, check out this `PR <https://github.com/kleinmann-orm/kleinmann-orm/pull/1001>`_
...
- Existing connection management interface and related public APIs which are deprecated:
 - `Kleinmann.get_connection`
 - `Kleinmann.close_connections`
Changed
^^^^^^^
- Refactored `kleinmann.transactions.get_connection` method to `kleinmann.transactions._get_connection`.
...
- Fix all logging to use Kleinmann's logger instead of root logger. (#879)
- Rename `db_client` logger to `kleinmann.db_client`.
...
- Replace pypika to `pypika-tortoise <https://github.com/kleinmann/pypika-tortoise>`_.
...
- Allow passing module objects to `models_paths` param of `Kleinmann.init_models()`. (#561)
...
- Add timezone support, **be careful to upgrade to this version**, see `docs <https://kleinmann-orm.readthedocs.io/en/latest/timezone.html>`_ for details. (#335)
...
0.16.13
-------
- Default install of ``kleinmann-orm`` now installs with no C-dependencies, if you want to use the C accelerators, please do a ``pip install kleinmann-orm[accel]`` instead.
- Added ``<instance>.clone()`` method that will create a cloned instance in memory. To persist it you still need to call ``.save()``
- ``.clone()`` will raise a ``ParamsError`` if kleinmann can't generate a primary key. In that case do a ``.clone(pk=<newval>)``
...
- Added ``app_label`` to ``test initializer(...)`` and ``KLEINMANN_TEST_APP`` as test environment variable.
...
0.16.5
------
* Moved ``Kleinmann.describe_model(<MODEL>, ...)`` to ``<MODEL>.describe(...)``
* Deprecated ``Kleinmann.describe_model()``
* Fix for ``generate_schemas`` param being ignored in ``kleinmann.contrib.quart.register_kleinmann``
...
* Added ``FastAPI`` helper that is based on the ``starlette`` helper but optionally adds helpers to catch and report with proper error ``DoesNotExist`` and ``IntegrityError`` Kleinmann exceptions.
...
* a Kleinmann ``PydanticModel`` now provides two extra helper functions:
...
.. caution::
   **This release drops support for Python 3.6:**
   ...
   Kleinmann ORM now requires a minimum of CPython 3.7
...
* Early Partial Init of models.
  ...
  .. code-block:: python3
    ...
    Kleinmann.init_models(["some.models", "other.ddef"], "models")

    # Now the models will have relationships built, so introspection of schema will be comprehensive

* Pydantic serialisation.

  We now include native support for automatically building a Pydantic model from Kleinmann ORM models.
...
Removals:
^^^^^^^^^
- Removed ``kleinmann.aggregation`` as this was deprecated since 0.14.0
...
- Removed support for Python 3.6 / PyPy-3.6, as it has been broken since 0.15.0
  ...
  If you still need Python 3.6 support, you can install ``kleinmann-orm<0.16`` as we will still backport critical bugfixes to the 0.15 branch for a while.
...
- One can now pass a PK value to a generated field, and Kleinmann ORM will use that as the PK as expected.
...
.. caution::
   **This release drops support of Python 3.5:**
   ...
   Kleinmann ORM now requires a minimum of CPython 3.6 or PyPy3.6-7.1
...
- Database functions ``Trim()``, ``Length()``, ``Coalesce()``, ``Lower()``, ``Upper()`` added to kleinmann.functions module.
...
Deprecations:
^^^^^^^^^^^^^
- ``import from kleinmann.aggregation`` is deprecated, please do ``import from kleinmann.functions`` instead.
...
0.13.3
------
- Fixed installing Kleinmann-ORM in non-unicode systems. (#180)
...
* Model schema now has a discovery API:
  ...
  One can call ``Kleinmann.describe_models()`` or ``Kleinmann.describe_model(<Model>)`` to get
  a full description of the model(s).

  Please see :meth:`kleinmann.Kleinmann.describe_model` and :meth:`kleinmann.Kleinmann.describe_models` for more info.
...
New Features:
^^^^^^^^^^^^^
- Example Sanic integration along with register_kleinmann hook in contrib (#163)
...
0.12.0
------
* Kleinmann ORM now supports non-autonumber primary keys.
...
* Added per-field indexes.
  ...
  When setting ``index=True`` on a field, Kleinmann will now generate an index for it.
...
0.10.10
-------
- Refactor ``Kleinmann.init()`` and test runner to not re-create connections per test, so now tests pass when using an SQLite in-memory database
...
- Added ``kleinmann.run_async()`` helper function to make smaller scripts easier to run. It cleans up connections automatically.
...
0.10.0
------
* Refactored ``Kleinmann.init()`` to init all connections and discover models from config passed
  as argument.
...
0.9.4
-----
- No more asserts, only Kleinmann Exceptions
...
* Added schema generation and more options for fields:
  ...
  .. code-block:: python3
      ...
      from kleinmann.utils import generate_schema

      client = SqliteClient(db_name)
      await client.create_connection()
      Kleinmann.init(client)
